...
const SearchBar = () => {
  ...
  const searchFor = async (query: string) => {
    try {
      ...
      const response = await fetch(`http://localhost:3001/games/?_limit=8`);
    ...
    <div ref={searchRef} className={styles.search}>
      <div className={styles.search__field}>
        <input
          ...
          value={query}
          onFocus={() => query.length>2 ? setIsVisible(true) : console.log()}
      ...
      {IsVisible && (
        <div className={styles.search__results}>
          {isLoading ? (
            ...
            results.map((result) => <SearchCard gameData={result} />)
          )}
        </div>
      )}
    </div>
