...
const GamesSlider = () => {
    ...
    const classImage = (idx: number) => {
        return idx === selectedIdx ? `${styles['carousel__item']} ${styles['carousel__item--selected']}` :
            idx === selectedIdx - 1 || (selectedIdx === 0 && idx === images.length - 1) ? `${styles['carousel__item']} ${styles['carousel__item--prev']}` :
                idx === selectedIdx + 1 || (selectedIdx === images.length - 1 && idx === 0) ? `${styles['carousel__item']} ${styles['carousel__item--next']}` :
                    idx === selectedIdx - 2 || (selectedIdx === 0 && idx === images.length - 2) || (selectedIdx === 1 && idx === images.length - 1) ? `${styles['carousel__item']} ${styles['carousel__item--prevLeftSecond']}` :
                        idx === selectedIdx + 2 || (selectedIdx === images.length - 2 && idx === 0) || (selectedIdx === images.length - 1 && idx === 1) ? `${styles['carousel__item']} ${styles['carousel__item--nextRightSecond']}` :
                            idx === selectedIdx - 3 || (selectedIdx === 0 && idx === images.length - 3) || (selectedIdx === 1 && idx === images.length - 2) || (selectedIdx === 2 && idx === images.length - 1) ? `${styles['carousel__item']} ${styles['carousel__item--hideLeft']}` :
                                idx === selectedIdx + 3 || (selectedIdx === images.length - 3 && idx === 0) || (selectedIdx === images.length - 2 && idx === 1) || (selectedIdx === images.length - 1 && idx === 2) ? `${styles['carousel__item']} ${styles['carousel__item--hideRight']}` :
    ...
    return (
        ...
        <div className={styles['carousel__head']}>
            ...
            <div className={styles['carousel__details']}>
                <p className={styles['carousel__all']}>Все ({images.length})</p>
                <div className={styles['carousel__action']}>
                    <button disabled={isButtonDisabled} className={styles['carousel__button']} id="prev" onClick={prevSlide}>
                        <p className="visually-hidden">Предыдущая игра</p>
                        <svg className={styles['carousel__arrow']} xmlns="http://www.w3.org/2000/svg" width="14" height="24" viewBox="0 0 14 24" fill="none">
                    ...
                    <div className={styles['carousel__dots']}>
                        {
                            dots.map(dot => 
                                <button key={dot} disabled={isButtonDisabled} className={selectDot === dot ? `${styles['carousel__button']} ${styles['carousel__button--active']}` : `${styles['carousel__button']} ${styles['carousel__button--circle']}`} id="step1" onClick={selectDot !== dot ? () => oneStep(dot) : undefined}>
                                    <p className={"visually-hidden"}>Один шаг на несколько объектов вперед</p>
                                </button>
                            )
                        }
                    </div>
                    <button disabled={isButtonDisabled} className={styles['carousel__button']} id="next" onClick={nextSlide}>
        ...
        <ul className={styles['carousel__list']}>
            {images.map((image, idx) => (
                <div key={idx} className={classImage(idx)} onClick={() => setItem(idx)}>
                    <img className={styles['carousel__image']} src={image.url} alt={`Slide ${idx + 1}`} />
                </div>
            ))}
        </ul>
    </div>
